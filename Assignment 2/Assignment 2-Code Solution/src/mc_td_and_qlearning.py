...
def generate_episode(env, policy, max_steps=500):
    ...
    for i in range(max_steps):
        ...
        action, reward, curr_state, done = take_one_step(env, policy, curr_state)
        episode.append((previous_state, action, reward))
        if done:
            break
    ############################
    return episode
...
def generate_returns(episode, gamma=0.9):
    ...
    reward = []
    for action, state, reward_i in episode:
        reward.append(reward_i)
    for i in range(len(episode)):
        gamma_value = []
        for k in range(len(episode) - i):
            gamma_value.append(gamma**k)
        epi_returns[i] = np.array(reward[i:]).dot(np.array(gamma_value))
    ############################
    return epi_returns
...
def mc_policy_evaluation(env, policy, Q_value, n_visits, gamma=0.9):
    ...
    ############################
    # YOUR IMPLEMENTATION HERE #
    for i, episode_i in enumerate(episode):
        S, A, _ = episode_i
        n_visits[S, A] = 1
        Q_value[S, A] = Q_value[S, A] + (returns[i] - Q_value[S, A])
    ############################
...
def epsilon_greedy_policy_improve(Q_value, nS, nA, epsilon):
    ...
    for state in range(nS):
        for action in range(nA):
            success = np.argwhere(Q_value[state] == np.amax(Q_value[state])).flatten().tolist()
            if action in success:
                len_success = len(success)
                new_policy[state][action] = new_policy[state][action] + (1 - epsilon)
                new_policy[state][action] = new_policy[state][action] / (len_success)
...
def td_sarsa(env, iterations=1000, gamma=0.9, alpha=0.1):
    ...
    for i in range(iterations):
        ...
        #2c
        #encourage agent to move forward by increasing reward by 1
        if s_t2 == 1:
            reward = 8
        elif s_t2 == 2:
            reward = 9
            ...
            reward = 12
        #negative rewards for states we want to avoid (holes)
        elif s_t2 == 7:
            reward = -100
        elif s_t2 == 9:
            reward = -100
            ...
            reward = -100
        #positive reward to reach the end
        elif s_t2 == 15:
            reward = 200
        a_t2 = sample_action(policy, s_t2)
        Q_value[s_t1][a_t1] = Q_value[s_t1][a_t1] + alpha * (reward + gamma * Q_value[s_t2][a_t2] - Q_value[s_t1][a_t1])
        policy = epsilon_greedy_policy_improve(Q_value, nS, nA, epsilon)
        k += 1
        epsilon = 1 / k
        a_t1, s_t1 = a_t2, s_t2
...
def qlearning(env, iterations=1000, gamma=0.9, alpha=0.1):
    ...
    for i in range(iterations):
        ...
        a_t2 = sample_action(policy, s_t2)
        desired_action = np.argmax(Q_value[s_t2])
        Q_value[s_t1][a_t1] += alpha * (reward + gamma * Q_value[s_t2][desired_action] - Q_value[s_t1][a_t1])
        policy = epsilon_greedy_policy_improve(Q_value, nS, nA, epsilon)
        k += 1
        epsilon = 1 / k
        a_t1, s_t1 = a_t2, s_t2
...
if __name__ == "__main__":
    ...
    Q_td, policy_td = td_sarsa(env, iterations=100000, gamma=0.5, alpha=0.4)
    test_performance(env, policy_td)
    # render_single(env, policy_td, 100) # uncomment to see a single episode

    print("\n" + "-" * 25 + "\nBeginning Q-Learning\n" + "-" * 25)
    Q_ql, policy_ql = qlearning(env, iterations=100000, gamma=0.5, alpha=0.4)

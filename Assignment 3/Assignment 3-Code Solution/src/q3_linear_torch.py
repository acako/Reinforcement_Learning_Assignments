...
class Linear(DQN):
    ...
    def update_target(self):
        ...
        parameters = self.q_network.state_dict()
        self.target_network.load_state_dict(parameters)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        q_val2 = torch.sum(q_values * torch.nn.functional.one_hot(actions, self.env.action_space.n), dim=1)
